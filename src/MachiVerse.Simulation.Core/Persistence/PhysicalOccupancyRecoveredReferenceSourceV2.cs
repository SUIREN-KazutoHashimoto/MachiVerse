...
/// <summary>
/// recovered physical.occupancy v2 の phase-1 reference source。
/// record id/schema に加えて record_kind を保持し、phase-2 の shape_ref target-kind closure に使う。
/// </summary>
public sealed class PhysicalOccupancyRecoveredReferenceSourceV2 : IDomainPartitionSnapshotReferenceSourceV1
{
    private readonly IReadOnlyDictionary<OpaqueId128, string> _recordKinds;

    public PhysicalOccupancyRecoveredReferenceSourceV2(
        IReadOnlyList<SnapshotSectionFragmentMaterialV1> fragments)
    {
        ...
        for (var index = 0; index < fragments.Count; index++)
        {
            ...
            if (!string.Equals(fragment.SectionId, PhysicalOccupancyRecordSchemaV2.PartitionId, StringComparison.Ordinal) ||
        ...
        RecordSchema = PhysicalOccupancyRecordSchemaV2.RecordSchema;
    ...
    public bool IsCollisionShape(OpaqueId128 recordId)
        => _recordKinds.TryGetValue(recordId, out var kind) &&
           string.Equals(kind, PhysicalOccupancyRecordSchemaV2.CollisionShapeKind, StringComparison.Ordinal);
    ...
    private static InvalidDataException Error(string suffix)
        => new($"persistence.snapshot.recovered-reference-{suffix}:physical.occupancy");
}
