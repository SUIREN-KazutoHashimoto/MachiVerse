...
/// <summary>society.market_transaction record schema 2.0 dedicated section provider.</summary>
public sealed class SocietyMarketTransactionSnapshotSectionProviderV2 : IDomainPartitionSnapshotSectionProviderV1
{
    private static readonly DomainPartitionIdentityV1 StandardIdentity =
        StandardDomainPartitionRegistry.Get(SocietyMarketTransactionRecordSchemaV2.PartitionId);

    public string SectionId => SocietyMarketTransactionRecordSchemaV2.PartitionId;
        ...
        IDomainRecordSchemaResolverV1? references = null)
    {
        ...
        var section = new CanonicalSnapshotSectionMaterialV1(
            SectionId,
            SectionSchema,
            market.ActualItemCount,
            market.Header.CanonicalDigest.ToArray(),
            fragments);
    ...
    private static IReadOnlyList<SnapshotSectionFragmentMaterialV1> BuildFragments(
        SocietyMarketTransactionSnapshotAuthorityV2 authority)
    {
        ...
        if (records.Count == 0)
        {
            var payload = SocietyMarketTransactionSnapshotFragmentWireV2.Encode(
                authority.Header,
        ...
        var groups = new List<SocietyMarketTransactionRecordMaterialV2[]>();
        var current = new List<SocietyMarketTransactionRecordMaterialV2>();
        var currentSize = baseSize;
        foreach (var record in records)
        {
            var recordBytes = SocietyMarketTransactionRecordWireCodecV2.Encode(record);
        ...
        if (groups.Count == 0 || groups.Count > uint.MaxValue)
            throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-count");
        ...
        for (var index = 0; index < groups.Count; index++)
        {
            ...
            var payload = SocietyMarketTransactionSnapshotFragmentWireV2.Encode(authority.Header, group);
        ...
        IDomainRecordSchemaResolverV1? references)
    {
        ...
        if (fragments.Count == 0)
            throw new InvalidDataException("persistence.snapshot.society-market-v2-section-fragment-missing");

        var records = new List<SocietyMarketTransactionRecordMaterialV2>();
        OpaqueId128? previous = null;
        ulong total = 0;
        for (var index = 0; index < fragments.Count; index++)
        {
            var fragment = fragments[index]
                ?? throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-null");
            if (!string.Equals(fragment.SectionId, SectionIdStatic, StringComparison.Ordinal) ||
                ...
                throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-shape");
            ...
            if (decoded.Records.Count != checked((int)fragment.ItemCount))
                throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-item-count");

            if (decoded.Records.Count == 0)
            {
                if (fragment.FirstRecordId is not null || fragment.LastRecordId is not null)
                    throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-empty-range");
            }
            else
            {
                ...
                if (fragment.FirstRecordId is null || fragment.LastRecordId is null ||
                    ...
                    throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-range");
            }

            foreach (var record in decoded.Records)
            {
                if (previous is { } prior && prior.CompareTo(record.RecordId) >= 0)
                    throw new InvalidDataException("persistence.snapshot.society-market-v2-fragment-record-order");
        ...
        if (total != expectedHeader.ItemCount || total != checked((ulong)records.Count))
            throw new InvalidDataException("persistence.snapshot.society-market-v2-restored-count");
    ...
    private static void ValidateCreatedSection(CanonicalSnapshotSectionMaterialV1 section)
    {
        if (!string.Equals(section.SectionId, SectionIdStatic, StringComparison.Ordinal) || section.SectionSchema != StandardIdentity.PartitionSchema)
            throw new InvalidDataException("persistence.snapshot.society-market-v2-section-identity");
        if (section.LogicalContentDigest.Length != 32)
            throw new InvalidDataException("persistence.snapshot.society-market-v2-section-digest");
        if (section.LogicalItemCount != section.Fragments.Aggregate(0UL, static (sum, fragment) => checked(sum + fragment.ItemCount)))
            throw new InvalidDataException("persistence.snapshot.society-market-v2-section-item-count");
        for (var index = 0; index < section.Fragments.Count; index++)
        {
            ...
            if (fragment.FragmentIndex != checked((uint)index) ||
                ...
                throw new InvalidDataException("persistence.snapshot.society-market-v2-section-fragment-shape");
        }
    }
    ...
    private static void RequireHeaderIdentity(PartitionStateHeaderV1 header)
    {
        if (header.PartitionId != StandardIdentity.PartitionId ||
            ...
            throw new InvalidDataException("persistence.snapshot.society-market-v2-verifier-header-identity");
    }
    ...
    private const string SectionIdStatic = SocietyMarketTransactionRecordSchemaV2.PartitionId;
}
