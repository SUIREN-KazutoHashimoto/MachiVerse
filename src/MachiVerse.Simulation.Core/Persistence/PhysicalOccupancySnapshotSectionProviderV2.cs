...
/// <summary>
/// physical.occupancy record schema 2.0 専用 section provider。
/// v1 generic provider を緩めず migration-aware composition から明示選択する。
/// </summary>
public sealed class PhysicalOccupancySnapshotSectionProviderV2 : IDomainPartitionSnapshotSectionProviderV1
{
    private static readonly DomainPartitionIdentityV1 StandardIdentity =
        StandardDomainPartitionRegistry.Get(PhysicalOccupancyRecordSchemaV2.PartitionId);

    public string SectionId => PhysicalOccupancyRecordSchemaV2.PartitionId;
        ...
        IDomainRecordSchemaResolverV1? references = null)
    {
        ...
        if (authority is not PhysicalOccupancySnapshotAuthorityV2 physical)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-provider-authority-type");
        physical.VerifyBoundAuthority();
        ValidateAuthorityReferences(physical, references);

        var fragments = BuildFragments(physical);
        var section = new CanonicalSnapshotSectionMaterialV1(
            SectionId,
            SectionSchema,
            physical.ActualItemCount,
            physical.Header.CanonicalDigest.ToArray(),
            fragments);
    ...
    private static IReadOnlyList<SnapshotSectionFragmentMaterialV1> BuildFragments(
        PhysicalOccupancySnapshotAuthorityV2 authority)
    {
        ...
        if (records.Count == 0)
        {
            var payload = PhysicalOccupancySnapshotFragmentWireV2.Encode(
                authority.Header,
                Array.Empty<PhysicalOccupancyRecordMaterialV2>());
        ...
        var groups = new List<PhysicalOccupancyRecordMaterialV2[]>();
        var current = new List<PhysicalOccupancyRecordMaterialV2>();
        var currentSize = baseSize;
        foreach (var record in records)
        {
            var recordBytes = PhysicalOccupancyRecordWireCodecV2.Encode(record);
        ...
        if (groups.Count == 0 || groups.Count > uint.MaxValue)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-count");
        ...
        for (var index = 0; index < groups.Count; index++)
        {
            ...
            var payload = PhysicalOccupancySnapshotFragmentWireV2.Encode(authority.Header, group);
        ...
        IDomainRecordSchemaResolverV1? references)
    {
        ...
        if (fragments.Count == 0)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-section-fragment-missing");

        var records = new List<PhysicalOccupancyRecordMaterialV2>();
        OpaqueId128? previous = null;
        ulong total = 0;
        for (var index = 0; index < fragments.Count; index++)
        {
            var fragment = fragments[index]
                ?? throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-null");
            if (!string.Equals(fragment.SectionId, SectionIdStatic, StringComparison.Ordinal) ||
                ...
                throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-shape");

            var decoded = PhysicalOccupancySnapshotFragmentWireV2.Decode(fragment.FragmentPayload);
            RequireSameHeader(expectedHeader, decoded.Header);
            if (decoded.Records.Count != checked((int)fragment.ItemCount))
                throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-item-count");

            if (decoded.Records.Count == 0)
            {
                if (fragment.FirstRecordId is not null || fragment.LastRecordId is not null)
                    throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-empty-range");
            }
            else
            {
                ...
                if (fragment.FirstRecordId is null || fragment.LastRecordId is null ||
                    ...
                    throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-range");
            }

            foreach (var record in decoded.Records)
            {
                if (previous is { } prior && prior.CompareTo(record.RecordId) >= 0)
                    throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-fragment-record-order");
                previous = record.RecordId;
                _ = PhysicalOccupancyPayloadCanonicalDigestV2.Compute(record.Payload, references);
                records.Add(record);
            }
        ...
        if (total != expectedHeader.ItemCount || total != checked((ulong)records.Count))
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-restored-count");
        if (expectedHeader.ItemCount == 0 && fragments.Count != 1)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-empty-fragment-count");

        var partition = new PhysicalOccupancyPartitionStateV2(records);
        if (references is not null) partition.RecordSet.ValidateTerrainSchemaReferences(references);
        var authority = new PhysicalOccupancySnapshotAuthorityV2(partition, expectedHeader);
        ...
        IDomainRecordSchemaResolverV1? references)
    {
        ...
        if (references is not null)
            authority.Partition.RecordSet.ValidateTerrainSchemaReferences(references);
    }

    private static void ValidateCreatedSection(CanonicalSnapshotSectionMaterialV1 section)
    {
        if (!string.Equals(section.SectionId, SectionIdStatic, StringComparison.Ordinal) ||
            section.SectionSchema != StandardIdentity.PartitionSchema)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-section-identity");
        if (section.LogicalContentDigest.Length != 32)
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-section-digest");
        if (section.LogicalItemCount != section.Fragments.Aggregate(
                0UL,
                static (sum, fragment) => checked(sum + fragment.ItemCount)))
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-section-item-count");
        for (var index = 0; index < section.Fragments.Count; index++)
        {
            ...
            if (fragment.FragmentIndex != checked((uint)index) ||
                ...
                throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-section-fragment-shape");
        }
    }
    ...
    private static void RequireHeaderIdentity(PartitionStateHeaderV1 header)
    {
        if (header.PartitionId != StandardIdentity.PartitionId ||
            ...
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-verifier-header-identity");
    }
    ...
    private static void RequireSameHeader(PartitionStateHeaderV1 expected, PartitionStateHeaderV1 actual)
    {
        if (expected.PartitionId != actual.PartitionId ||
            ...
            throw new InvalidDataException("persistence.snapshot.physical-occupancy-v2-restored-header");
    }
    ...
    private const string SectionIdStatic = PhysicalOccupancyRecordSchemaV2.PartitionId;
}
