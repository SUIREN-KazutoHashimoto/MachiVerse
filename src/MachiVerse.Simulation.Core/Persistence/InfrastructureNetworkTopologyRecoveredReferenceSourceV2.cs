...
/// <summary>
/// Recovered infrastructure.network_topology /2.0 phase-1 reference source.
/// Record kinds are retained so phase-2 can enforce network/node/edge target-kind closure.
/// </summary>
public sealed class InfrastructureNetworkTopologyRecoveredReferenceSourceV2 : IDomainPartitionSnapshotReferenceSourceV1
{
    private readonly IReadOnlyDictionary<OpaqueId128, string> _recordKinds;

    public InfrastructureNetworkTopologyRecoveredReferenceSourceV2(
        IReadOnlyList<SnapshotSectionFragmentMaterialV1> fragments)
    {
        ArgumentNullException.ThrowIfNull(fragments);
        if (fragments.Count == 0) throw Error("fragment-missing");

        var standard = StandardDomainPartitionRegistry.Get(InfrastructureNetworkTopologyRecordSchemaV2.PartitionId);
        ...
        for (var index = 0; index < fragments.Count; index++)
        {
            ...
            if (!string.Equals(fragment.SectionId, InfrastructureNetworkTopologyRecordSchemaV2.PartitionId, StringComparison.Ordinal) ||
            ...
            if (decoded.Records.Count != checked((int)fragment.ItemCount)) throw Error("item-count");
            if (decoded.Records.Count == 0)
            {
                if (fragment.FirstRecordId is not null || fragment.LastRecordId is not null) throw Error("range-empty");
            }
            else
            {
        ...
        if (total != Header.ItemCount || total != checked((ulong)ids.Count)) throw Error("total-count");
        if (Header.ItemCount == 0 && fragments.Count != 1) throw Error("empty-fragment-count");

        PartitionId = standard.PartitionId;
        RecordSchema = InfrastructureNetworkTopologyRecordSchemaV2.RecordSchema;
    ...
    public bool IsNetwork(OpaqueId128 recordId)
        => IsKind(recordId, InfrastructureNetworkTopologyRecordSchemaV2.NetworkKind);

    public bool IsNode(OpaqueId128 recordId)
        => IsKind(recordId, InfrastructureNetworkTopologyRecordSchemaV2.NodeKind);

    public bool IsEdge(OpaqueId128 recordId)
        => IsKind(recordId, InfrastructureNetworkTopologyRecordSchemaV2.EdgeKind);

    private bool IsKind(OpaqueId128 recordId, string kind)
        => _recordKinds.TryGetValue(recordId, out var actual) && string.Equals(actual, kind, StringComparison.Ordinal);
    ...
    private static InvalidDataException Error(string suffix)
        => new($"persistence.snapshot.recovered-reference-{suffix}:infrastructure.network_topology");
}
