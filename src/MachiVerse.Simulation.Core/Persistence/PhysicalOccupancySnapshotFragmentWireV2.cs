...
public sealed record PhysicalOccupancySnapshotFragmentDecodedV2(
    PartitionStateHeaderV1 Header,
    IReadOnlyList<PhysicalOccupancyRecordMaterialV2> Records);

/// <summary>
/// physical.occupancy v2 の section fragment envelope。
/// field 1 は既存 PartitionStateHeaderV1、repeated field 2 は v2 record wire。
/// </summary>
public static class PhysicalOccupancySnapshotFragmentWireV2
{
        ...
        IReadOnlyList<PhysicalOccupancyRecordMaterialV2> records)
    {
        ...
        foreach (var record in records)
        {
            ...
            if (record.RecordSchema != PhysicalOccupancyRecordSchemaV2.RecordSchema)
            ...
            WriteMessage(stream, 2, PhysicalOccupancyRecordWireCodecV2.Encode(record));
        }
        return stream.ToArray();
    }

    public static PhysicalOccupancySnapshotFragmentDecodedV2 Decode(ReadOnlySpan<byte> encoded)
    {
        ...
        while (!reader.End)
        {
            ...
            switch (field)
            {
                ...
                case 2:
                {
                    ...
                    recordSeen = true;
                    reader.RequireWire(wire, 2);
                    var record = PhysicalOccupancyRecordWireCodecV2.Decode(reader.ReadBytes());
                    if (record.CreatedStep > header.BasisStep || record.RetiredStep is { } retired && retired > header.BasisStep)
                        throw WireError("record-step-after-basis");
                    if (previous is { } prior && prior.CompareTo(record.RecordId) >= 0)
                        throw WireError("record-order");
        ...
        return new PhysicalOccupancySnapshotFragmentDecodedV2(
            header,
    ...
    {
        var identity = StandardDomainPartitionRegistry.Get(PhysicalOccupancyRecordSchemaV2.PartitionId);
    ...
    private static InvalidDataException WireError(string suffix)
        => new($"persistence.snapshot.physical-occupancy-v2-fragment:{suffix}");
