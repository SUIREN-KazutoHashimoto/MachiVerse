...
public sealed record SocietyMarketTransactionSnapshotFragmentDecodedV2(
    PartitionStateHeaderV1 Header,
    IReadOnlyList<SocietyMarketTransactionRecordMaterialV2> Records);

/// <summary>society.market_transaction v2 section fragment envelope.</summary>
public static class SocietyMarketTransactionSnapshotFragmentWireV2
{
        ...
        IReadOnlyList<SocietyMarketTransactionRecordMaterialV2> records)
    {
        ...
        foreach (var record in records)
        {
            ...
            WriteMessage(stream, 2, SocietyMarketTransactionRecordWireCodecV2.Encode(record));
        }
        return stream.ToArray();
    }

    public static SocietyMarketTransactionSnapshotFragmentDecodedV2 Decode(ReadOnlySpan<byte> encoded)
    {
        ...
        while (!reader.End)
        {
            ...
            switch (field)
            {
                ...
                case 2:
                    ...
                    recordSeen = true;
                    reader.RequireWire(wire, 2);
                    var record = SocietyMarketTransactionRecordWireCodecV2.Decode(reader.ReadBytes());
                    if (record.CreatedStep > header.BasisStep || record.RetiredStep is { } retired && retired > header.BasisStep)
                        throw Error("record-step-after-basis");
                    if (previous is { } prior && prior.CompareTo(record.RecordId) >= 0)
                        throw Error("record-order");
        ...
        return new SocietyMarketTransactionSnapshotFragmentDecodedV2(header, Array.AsReadOnly(records.ToArray()));
    }

    private static void RequireHeader(PartitionStateHeaderV1 header)
    {
        var identity = StandardDomainPartitionRegistry.Get(SocietyMarketTransactionRecordSchemaV2.PartitionId);
    ...
    private static InvalidDataException Error(string suffix)
        => new($"persistence.snapshot.society-market-v2-fragment:{suffix}");
