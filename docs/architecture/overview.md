...
| 領域 | 主目的 | 主な責務 |
|---|---|---|
| シミュレーションコア | 世界シミュレーションの実行 | 状態保持、30Hz計算、時間進行、エージェント・環境の状態遷移、ルール実行 |
| ゲートウェイ | 外部との接続・認可・緩衝境界 | 状態キャッシュ、約1秒遅延バッファ、利用者認証・認可、操作仲介、管理要求仲介、スケールアウト |
...
ゲートウェイは権限外の要求をコアへ伝達しません。コアはゲートウェイで認可済みでも、シミュレーション上の妥当性を最終判定します。
