...
一般ビューは **Web アプリケーションとして提供**し、フル3Dのシミュレーション世界の描画には **Three.js** を使用します。
...
VR等は、将来的にこの住民体験を強化するアドオンとして採用する可能性があります。ただし、第二原則を満たすためにVRや身体トラッキング等を必須とするものではありません。具体的な追加提示技術、対応デバイス、操作方式、身体表現、視点、感覚表現は別途要件として定義します。
...
一般ビューはWeb上で動作し、シミュレーション世界の3D描画に **Three.js** を使用します。

Three.jsはビューにおける描画技術であり、シミュレーション状態の正本や世界の因果計算を担当しません。フル3Dの世界状態そのものはシミュレーションコアが管理し、ビューはゲートウェイから公開された情報をもとに描画します。

数値、一覧、グラフ、状態表示など、3D描画以外の情報表現も必要に応じて併用できます。

具体的なThree.jsのバージョン、WebGL/WebGPUの利用方針、シーン構造、レンダリング方式、LOD、アセット形式、シェーダー、描画更新頻度、UIフレームワーク等は今後の詳細設計で定義します。
...
- 大量データ受信時にも UI やThree.js描画が操作不能にならない構造を検討する。
...
- Three.jsの描画結果をシミュレーション状態の正本として扱うこと
...
- Three.jsのバージョンと更新方針
- WebGL/WebGPUの利用方針
- Three.jsのシーン・レンダリング・LOD・アセット・シェーダー設計
- UIフレームワーク等、Three.js以外のWeb技術スタック
- データ取得単位
- 大量データの3D可視化方式
- 対応端末・対応ブラウザ等の実行環境